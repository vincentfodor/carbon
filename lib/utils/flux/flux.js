...
function connect(ComposedView, stores) {
  ...
  function (_ComposedView) {
    ...
    function View() {
      ...
      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "_onChange", function (key) {
        // update the state with the data for the store that changed
        _this.setState(_defineProperty({}, key, _stores[key].getState()));
      });

      _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "_getStoreStates", function () {
        var states = {};
