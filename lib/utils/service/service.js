...
var Service = function () {
  ...
  function Service() {
    ...
    this.get = function (id, options, _onError) {
      var request = void 0;

      if (typeof options === 'function') {
        _logger2.default.deprecate('Passing onSuccess and onError as seperate arguments is deprecated. Please use `get(1, { onSuccess: function, onError: function })`'); // eslint-disable-line max-len

        request = _this.client.get(id);

        request.then(_this.handleResponse.bind(_this, options), _this.handleResponse.bind(_this, _onError));
      } else {
        if (typeof options.query === 'undefined') {
          options.query = {};
        }

        options.query = (0, _serialize2.default)(options.query);
        request = _this.client.get(id, options.query);

        request.then(_this.handleResponse.bind(_this, options.onSuccess), _this.handleResponse.bind(_this, options.onError));
      }
    };
