    ...
    function Component() {
      ...
      _this.positionMessage = function () {
        if (!_this.state.valid || _this.state.warning || _this.state.info) {
          ...
          if (icon && message && message.offsetHeight) {
            ...
            var messageScreenPosition = message.getBoundingClientRect().left + messageOffsetWidth;

            var shouldFlip = false;

            // change the position if it is offscreen
            if (_this.context.modal && _this.context.modal.getDialog()) {
              // if in a modal check its position relative to that
              var dialog = _this.context.modal.getDialog();
              ...
              shouldFlip = messageScreenPosition > _this._window.innerWidth;
            }
      ...
      _this._handleContentChange = function () {
        ...
        if (!_this.state.valid || _this.state.warning || _this.state.info) {
          ...
          _this.setState({ errorMessage: null, messageShown: false, valid: true, warning: false, info: false });
        }
      };
