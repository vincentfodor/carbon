...
var ElementResize = {
  ...
  addListener: function addListener(element, callback) {
    ...
    if (!element.__resizeTrigger__) {
      element.__resizeListenerCallbacks__ = []; // ensure that the element has position relative for the child object to work properly

      if (_browser.default.getWindow().getComputedStyle(element).position === 'static') {
        element.style.position = 'relative';
      } // creates an object which will support the resize event listener


      var obj = _browser.default.getDocument().createElement('object');
      ...
      obj.type = 'text/html';
      var isMS = _bowser.default.msie || _bowser.default.msedge;
  ...
  removeListener: function removeListener(element, callback) {
    if (element.__resizeListenerCallbacks__) {
      ...
      element.__resizeListenerCallbacks__.splice(element.__resizeListenerCallbacks__.indexOf(callback), 1); // if there are no event listeners left, time to detach the event
...
var resizeListener = function resizeListener(ev) {
  var trigger = ev.target.__resizeTrigger__; // using the reference to the source element, retrieve the array of callbacks and trigger them all
...
var objectLoad = function objectLoad(obj, element) {
  return function () {
    ...
    if (view) {
      ...
      view.__resizeTrigger__ = element; // apply the event listener to the new object's document
