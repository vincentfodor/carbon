...
var I18nHelper = {
  ...
  format: function format(locale) {
    return {
      delimiter: _i18nJs.default.t('number.format.delimiter', {
        defaultValue: ','
      }),
      separator: _i18nJs.default.t('number.format.separator', {
        defaultValue: '.'
      }),
      unit: _i18nJs.default.t('number.currency.format.unit', {
        locale: locale,
        defaultValue: '£'
      }),
      format: _i18nJs.default.t('number.currency.format.format', {
        locale: locale,
        defaultValue: '%u%n'
      })
    };
  ...
  formatDecimal: function formatDecimal() {
    ...
    var locale = options.locale || _i18nJs.default.locale || 'en';
    var format = I18nHelper.format(locale);
    return _i18nJs.default.toNumber(valueToFormat, {
      precision: precision,
  ...
  abbreviateNumber: function abbreviateNumber(num) {
    ...
    if (absolute > 949 && absolute < 999950) {
      var translation = _i18nJs.default.t('number.format.abbreviations.thousand', {
        defaultValue: 'k'
      });

      return "".concat(I18nHelper.roundForAbbreviation(num, 100, options)).concat(translation);
    }

    if (absolute > 999949) {
      var _translation = _i18nJs.default.t('number.format.abbreviations.million', {
        defaultValue: 'm'
      });

      return "".concat(I18nHelper.roundForAbbreviation(num, 100000, options)).concat(_translation);
    }

    return "".concat(I18nHelper.formatDecimal(num, 2, options));
  },
  ...
  humanizeFilesize: function humanizeFilesize(number) {
    ...
    var i = Math.floor(Math.log(number) / Math.log(k)); // Unable to use the exponentiation operator ** as browserify
    ...
    return "".concat(filesize, " ").concat(sizes[i]);
  },
  ...
  roundForAbbreviation: function roundForAbbreviation(number, divisor) {
    var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    var num = new _bignumber.default(number);
    var div = new _bignumber.default(divisor);
  ...
  formatCurrency: function formatCurrency() {
    ...
    var unit = options.unit || format.unit;
    var structure = options.format || format.format; // Checking explicitly as 0 is a valid precision

    if (typeof precision === 'undefined' || precision === null) {
      precision = 2;
    }

    return _i18nJs.default.toCurrency(valueToFormat, {
      precision: precision,
      delimiter: format.delimiter,
      separator: format.separator,
      unit: unit,
