...
var Form = function (_React$Component) {
  ...
  function Form() {
    ...
    }, _this.setIsDirty = function () {
      if (!_this.state.isDirty) {
        _this.setState({ isDirty: true });
      }
    }, _this.resetIsDirty = function () {
      if (_this.state.isDirty) {
        _this.setState({ isDirty: false });
      }
    ...
    }, _this.addUnsavedWarningListener = function () {
      _this._window.addEventListener('beforeunload', _this.checkIsFormDirty);
    }, _this.removeUnsavedWarningListener = function () {
      _this._window.removeEventListener('beforeunload', _this.checkIsFormDirty);
    }, _this.checkIsFormDirty = function (ev) {
      if (_this.state.isDirty) {
        // Confirmation message is usually overridden by browsers with a similar message
        var confirmationMessage = _i18nJs2.default.t('form.save_prompt', { defaultValue: 'Do you want to leave this page? Changes that you made may not be saved.' });
        ev.returnValue = confirmationMessage; // Gecko + IE
        return confirmationMessage; // Gecko + Webkit, Safari, Chrome etc.
      }
    }, _this._document = _browser2.default.getDocument(), _this._window = _browser2.default.getWindow(), _this.inputs = {}, _this.errorCount = 0, _this.warningCount = 0, _this.incrementErrorCount = function () {
      _this.errorCount += 1;
...
Form.propTypes = {
   ...
   * @default true
   */
  unsavedWarning: _propTypes2.default.bool,
   ...
   * @default false
   */
  autoDisable: _propTypes2.default.bool,
